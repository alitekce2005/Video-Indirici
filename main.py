...
async def clip_video(url: str, start: str = "0:00", end: str = "", quality: str = "720"):
    ...
    ffmpeg_cmd = ["ffmpeg", "-y", "-i", raw_file, "-ss", start_tc]
    if end_tc:
...
async def download_thumbnail(url: str):
    """Video kapak fotoğrafını indir ve döndür."""
    info_opts = {
        'quiet': True,
        'skip_download': True,
        'noplaylist': True,
    ...
    try:
        ...
        if not thumbnail_url:
            raise HTTPException(status_code=404, detail="Kapak fotoğrafı bulunamadı.")

        async with httpx.AsyncClient(timeout=15) as client:
            resp = await client.get(thumbnail_url)
            if resp.status_code != 200:
                raise HTTPException(status_code=502, detail="Kapak fotoğrafı indirilemedi.")

        raw_title = info.get('title') or 'thumbnail'
        safe_title = "".join([c for c in raw_title if c.isalnum() or c in " _-"]).rstrip()
        ...
        raise HTTPException(status_code=400, detail=f"Thumbnail alınamadı: {str(e)}")
