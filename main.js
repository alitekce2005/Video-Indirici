...
async function startDownload() {
    ...
    // YENİ EKLENEN KISIM: Eğer kullanıcı Fotoğraf seçtiyse doğrudan o fonksiyona git
    if (selectedOpt === 'photo') {
    ...
    try {
        ...
        if (window._previewData && window._previewData.directDownloadUrl) {
            ...
            if (!contentType || !contentType.includes("application/json")) {
                throw new Error("Sunucu ile bağlantı kurulamadı veya backend kapalı (API Hatası).");
            }
...
/* ---------- PWA Kurulum (Uygulamayı Yükle Butonu) ---------- */
let deferredPrompt;

// Tarayıcı PWA kurulumuna hazır olduğunda bu olayı yakalayıp değişkene atıyoruz
window.addEventListener('beforeinstallprompt', (e) => {
    e.preventDefault(); // Tarayıcının varsayılan çubuğunu engelle
    deferredPrompt = e; // Bizim buton için kaydet
});

async function installApp(e) {
    if (e) e.preventDefault();
    closeMobileMenuIfOpen(); // Mobil menüyü de otomatik kapatalım

    if (deferredPrompt) {
        // Kaydettiğimiz kurulum ekranını (PWA Install Prompt) tetikle
        deferredPrompt.prompt();
        // Kullanıcının kabul edip etmediğini bekle
        const { outcome } = await deferredPrompt.userChoice;
        if (outcome === 'accepted') {
            console.log('Kullanıcı uygulamayı kurdu!');
        }
        // Kurulum ekranı sadece bir kez gösterilebilir, bu yüzden sıfırlıyoruz
        deferredPrompt = null;
    } else {
        // Eğer kurulum ekranı yoksa (Uygulama zaten yüklüyse veya cihaz iOS ise)
        const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream;
        if (isIOS) {
            // Apple Safari otomatik kurulum butonlarına izin vermez, yönlendirme yaparız
            showToast('iOS için: Alttaki "Paylaş" butonuna basıp "Ana Ekrana Ekle"yi seçin.', 'blue');
        } else {
            showToast('Uygulama cihazınızda zaten yüklü.', 'green');
        }
    }
}